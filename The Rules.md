# The Rules
You start with one random traveler and all first "rank" areas and must find your other Travelers, Chapters and Areas.
### --Region Gating--
You are initailly limited to the first 8 towns in the game and their surrounding areas and must find access to the second and third ranks as items. You also may not access optional dungeons without the required map of the area.

The Ranks of the areas are simple to deduce by their recommended level.

Rank 1: Levels 1 to 13\
Rank 2: Levels 14 to 36\
Rank 3: Level 37 and above

Each dungeon that is not associated with a character story or crossed paths chapter requires a map to access.
### --Story Progression--
Each chapter of each character's story requires a "Progressive Chapter" for that character, where the next chapter in that characters story is available to play. Split chapters, such as Temenos's Chapter 3, count as two seperate chapters for item requirements and go in order from lowest to highest recommended level. Completing a new chapter sends off a check. This does not apply to character's first chapters and Crossed Path chapters, which can be completed whenever they're available but do not provide a check when beaten.

Exceptions:\
Osvald's first two chapters are played one after another and thus both count as his first chapter, providing no check and requiring nor progressive chapters.\
Throné's chapter 3 is a split chapter where both the chapters have the same recommended level. These two both become available with the same progressive chapter and can both be beaten.

### --Skills and Licenses--
Acquiring Licenses and learning job skills and support skills you don't have as items must be done but any skills you have learned but not gotten as items cannot be used. Same with licenses and proofs. This doesn't apply to the first two skill each job starts with, which can be used freely. You can learn job skills you arent allowed to use to learn support skills you can use.

### --Other Stuff--
Learned Skill Slots and Beast Lore Slots: You are not allowed to learn skills with Hikari and capture monsters with Ochette until you acquire at least one slot for their talent with the exception of Ochette's first chapter where you can capture the lizard without a slot to progress the game.

Scent of Commerce: The three scent of commerce quests are also items. You need to complete one to complete Partitio's third chapter and you need to complete the Tropu'hopu one to access the Sundering Sea.
